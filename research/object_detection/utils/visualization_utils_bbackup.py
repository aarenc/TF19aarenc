  ...
  for box, color in box_to_color_map.items():
    ...
    if keypoints is not None:
      draw_keypoints_on_image_array(
          ...
          use_normalized_coordinates=use_normalized_coordinates)
  dict['Fecha'] = datetime.now().strftime("%Y-%m-%d")
  #dict['Hora'] = datetime.now().strftime("%H:%M:%S")
  print(dict)
 # fecha= zip(datetime.now().strftime("%Y-%m-%d")+","+[dict])
 # pepe = "variable global para acceder desde el notebook principal"
  with open("../text.csv", "a") as archivocsv:
    bankwriter = csv.writer(archivocsv, delimiter=",", lineterminator="\n")
    bankwriter.writerow([dict])
  return image
